...
inputEx.DateSelectMonthField = function (options) {
   ...
   for (i = 0 ; i < 3 ; i += 1) {
      ...
      if (i === this.dayIndex) {
         options.fields.push({ type: 'string', placeholder: this.messages.dayPlaceholder, size: 2 });
      }
      else if (i === this.yearIndex) {
         options.fields.push({ type: 'string', placeholder: this.messages.yearPlaceholder, size: 4 });
      }
      else {
