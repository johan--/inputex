...
inputEx.DateSplitField = function(options) {
   ...
   for(var i = 0 ; i < 3 ; i++) {
      if(i == this.dayIndex) {
         options.fields.push({type: 'integer', placeholder: this.messages.dayPlaceholder, size: 2, trim: true });
      }
      else if(i == this.yearIndex) {
         options.fields.push({type: 'integer', placeholder: this.messages.yearPlaceholder, size: 4, trim: true });
      }
      else {
         options.fields.push({type: 'integer', placeholder: this.messages.monthPlaceholder, size: 2, trim: true });
      }
   }
